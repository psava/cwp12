...
We can see in Figure~\ref{fig:imgs-b_r-b05-150} the interaction between the transmitted source wavefield $\USt$, shown in Figure~\ref{wts-b05-150}, with 
...
\beq
 J_{\tau}({\bf m})= \frac{1}{2} \lnorm{P_{\tau}\left [R^{tr}(\xx,\tau)+R^{rt}(\xx,\tau)\right ]}^2_2,
\label{eq:of1}
\eeq 
and for the space-lag gathers,
\beq
 J_{\lambda_x}({\bf m})= \frac{1}{2} \lnorm{P_{\tau}\left [R^{tr}(\xx,\lambda_x)+R^{rt}(\xx,\lambda)\right ]}^2_2.
\label{eq:of2}
\eeq 
%
...
\beq
J_{\bf c}({\bf m}) =  \frac{1}{2} \lnorm{P(\hh,\tau)\left [R^{tr}(\hh,\tau)+R^{rt}(\hh,\tau)\right ]}^2_2.
\eeq
%
...
CIP with correct velocity as penalty function. \red{Altough we are using a correct image for the penalty operator, the OF is minimum at an error
of 3\% as shown in Figure~\ref{fig:OF_cip}}.
