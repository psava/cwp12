...
contrast. In~\rfg{img05_ref} we show the common imaging condition for one shot at $x=5km$. We can see in this image strong 
...
reflected source wavefield behaves as the receiver wavefield and its energy maps toward the receivers. We can see after
 the imaging process is finished, shown in~\rfg{img05_ref}, that the backscattered energy is maximum near the critical
...
~\rfg{img05_nn} shows the cross-correlation between transmitted wavefields, an image which is due to the
earth reflectivity.~\rfgs{img05_nb} and~\ref{fig:img05_bn} show the images $\Rc{tr}$ and $\Rc{rt}$ corresponding
...
in~\rfg{img05_bb}, shows additional contribution to the reflectivity of the earth due to the cross-correlation
...
for the time-lag gather shown in~\rfg{cit05_ref1}.~\rfg{cit05_nn} shows the image $\Rct{tt}$,
...
Above the reflector depth, the slope is controlled by the medium velocity of layer 1, whereas below the interface the slope is controlled
 by the velocity of the layer 2.~\rfgs{cit05_nb} and \ref{fig:cit05_bn} show the backscattered
...
%
%
\rfg{cit05_bb} shows the $\Rct{rr}$ image, in this case the source wavefield is going in upward
...
imaging is correct. 
%
\rfg{citx05_nn} shows the $\Rcl{tt}$ image, where we see the energy correctly focused at $\lambda_x=0$. 
\rfgs{citx05_bn} and~\ref{fig:citx05_nb} show the backscattered events $\Rcl{rt}$ and $\Rcl{tr}$ in the space-lag gathers, 
which also map toward $\lambda_x=0$.
%
\rfg{citx05_bb} shows the image coming from the reflected wavefields $\Rcl{rr}$, in this case the events are
...
\rfg{cip05_nn} shows the CIP for the transmitted wavefields $\Rce{tt}$. We see the energy focusing at zero lag 
...
are mapped for negative $\tau$.~\rfg{cip05_nb}
...
The cip from the reflected wavefields shows weak energy with the same pattern of~\rfg{cip05_nn}.
