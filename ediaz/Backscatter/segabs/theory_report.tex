...
of the source wavefield. Likewise,~\rfgs{wtr-b05-150},~\ref{fig:wtr-b05-275} and~\ref{fig:wtr-b05-500}
...
~\rfgs{imgs-b_r-b05-150},~\ref{fig:imgs-b_r-b05-275} and~\ref{fig:imgs-b_r-b05-500} show the accumulated image as a function of time (integration over time of 
the product between wavefields).

~\rfg{imgs-b_r-b05-150} show the interaction between the transmitted source wavefield $\USt$, shown in \rfg{wts-b05-150}, and 
...
%
 ...
 The reflected source wavefield, shown in~\rfg{wts-b05-275}, generates new backscattered events
...
reflected source wavefield behaves as the receiver wavefield and its energy maps toward the receivers. We can see that after
 the imaging process is finished,~\rfg{img05_ref1}, that the backscattered energy is maximum near the critical
...
~\rfg{img05_nn1} shows the cross-correlation between transmitted wavefields, producing an image due to the
...
, shown in~\rfg{img05_bb1}, contains additional contribution to the reflectivity of the earth due to the cross-correlation
...
Illustration of the linearity of the conventional imaging condition. We can split the conventional image, Figure~\ref{fig:img05_ref1}, in four %
...
for the time-lag gather shown in~\rfg{cit05_ref1}.~\rfg{cit05_nn1} shows the image $\Rct{tt}$ with
...
Above the reflector depth, the slope is controlled by the velocity of layer 1, whereas below the interface the slope is controlled
 by the velocity of layer 2.~\rfgs{cit05_nb1} and \ref{fig:cit05_bn1} show the backscattered
...
%
%
\rfg{cit05_bb1} shows the $\Rct{rr}$ image; in this case the source wavefield is going in the upward
...
imaging is correct. 
%
\rfg{citx05_nn1} shows the $\Rcl{tt}$ image with the energy correctly focused at $\lambda_x=0$. 
...
\rfg{cip05_nn1} shows the CIP for the transmitted wavefields $\Rce{tt}$. The energy focuses at zero lag 
for the $\tau-\lambda_x$ panel. The $\lambda_z-\tau$ shows a kink produced by the abrupt change in velocity for the primaries, which
are mapped at negative $\tau$.~\rfg{cip05_nb1} shows the $\Rce{tr}$ image, and we can see a change in the $\lambda_z-\tau$ plane, 
where the backscattered energy is mapped to positive lags.~\rfg{cip05_bn1} shows the complementary backscattered energy
 that is mapped to negative $\lambda_z$ and positive $\tau$ lags.
The CIP from the reflected wavefields,~\rfg{cip05_bb1}, shows weak energy concentrated at zero lags.
...
Model error sensitivity with CIP gathers: (a) -12\%, (b) -9\%, (c) -6\%, (d) -3\%, (e) +0\%, (f) +3\%, %
(g) +6\%, (h) +9\% and (i) +12\% velocity perturbation in the top layer.}
...
%
Finally,~\rfgs{cip02_ref} to~\ref{fig:cip09_ref} show the sensitivity of CIPs to velocity errors. For incorrect velocity, the events
...
\beq
 J_{\lambda_x}= \frac{1}{2} \lnorm{P(\lambda_x)\left [R^{tr}(\xx,\lambda_x)+R^{rt}(\xx,\lambda_x)\right ]}^2_2.
\label{eq:of2}
\eeq 
%
...
~\rFgs{Penalty_cit} to~\ref{fig:Penalty_cip} show the penalty functions for time-lag, space-lag and CIP gathers respectively.
The objective functions for our synthetic example are shown in~\rfgs{OF_cit} to~\ref{fig:OF_cip} for time-lag gathers, space-lag gathers and
...
\multiplotcol{1}{Penalty_cit,Penalty_cix,Penalty_cip}{height=0.2\textheight}{Penalty functions for time-lag gathers (a), space-lag gathers (b) and
CIP gathers. Grey and white colors represent low and high penalty, respectively.}



\multiplotcol{1}{OF_cit,OF_cix,OF_cip}{height=0.2\textheight}{Normalized objective functions for time-lag gathers $ J_{\tau}$ (a), space-lag gathers $ J_{\lambda_x}$ (b) and
CIP gathers $J_{\bf c}$.}
