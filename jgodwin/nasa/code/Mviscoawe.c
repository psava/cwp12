...
{
    bool verb,fsrf,snap,expl,dabc,abcone,is2D,cfl,rtm; 
...
about being uninitialized */
    ...
    if(! sf_getbool("rtm",&rtm)) rtm=false; /* Make an image using RTM with the given data, run the source wavefield forward, save snapshots to disk, then run the receiver wavefield backwards, correlate and output image*/
    ...
    /*------------------------------------------------------------*/
    char *tmpwfldname = sf_charalloc(1024);
    /*------------------------------------------------------------*/
    /* I/O files */
    ...
    Fden = sf_input ("den"); /* damping parameter*/
    if (snap) Fwfl = sf_output("wfl"); /* wavefield */

    sf_file Fswfld = NULL;
    sf_file Frecwav = NULL;
    if(rtm) {
        Fswfld = sf_tempfile(&tmpwfldname,"rwb");
        Frecwav = sf_input("adj");

        sf_warning("saving source wavefield to %s",tmpwfldname);
    }
...
//    vt  =sf_floatalloc3(fdm->nzpad,fdm->nxpad,fdm->nypad); 
    ...
    for(iy=NOP; iy<fdm->nypad-NOP; iy++) {
        for(ix=NOP; ix<fdm->nxpad-NOP; ix++) {
            for(iz=NOP; iz<fdm->nzpad-NOP; iz++) {
                ro[iy][ix][iz] = 1.0f-ro[iy][ix][iz]*dt;
            }
        }
    }
    ...
    float vpmin = vp[0][0][0]; float vpmax = vp[0][0][0];
    for        (iy=0; iy<fdm->nypad; iy++) {
        for    (ix=0; ix<fdm->nxpad; ix++) {
            for(iz=0; iz<fdm->nzpad; iz++) {
                ...
                vp[iy][ix][iz] = vp[iy][ix][iz] * vp[iy][ix][iz] * dt*dt;
            }
        }
    }
    ...
    if (cfl){
        float dmax;
        if (dz > dx && dz > dy) dmax = dz;
        else if (dy > dx && dy > dz) dmax = dy;
        else if (dx > dy && dx > dz) dmax = dx;

        if ((dt*vpmax)/dx+(dt*vpmax)/dz+(dt*vpmax)/dy >= sqrt(2.0))
            sf_error("Failed stability check");
        if (vpmin/fmax/dmax < NUM_INTERVALS) 
            sf_warning("Failed accuracy check");
    }
    ...
    if (dabc)	spo = sponge_make(fdm->nb);
...
#pragma omp parallel for					\
    schedule(dynamic)				\
    private(ix,iy,iz)						\
    shared(fdm,ua,uo,co,cax,cay,caz,cbx,cby,cbz)
	for (iy=NOP; iy<fdm->nypad-NOP; iy++) {
	    for(ix=NOP; ix<fdm->nxpad-NOP; ix++) {
            for(iz=NOP; iz<fdm->nzpad-NOP; iz++) {
    ...
    stencil_time += omp_get_wtime()-time;
   ...
   injection_time += omp_get_wtime()-time;
...
#pragma omp parallel for	    \
	...
	for(iy=0; iy<fdm->nypad; iy++) {
	    for(ix=0; ix<fdm->nxpad; ix++) {
            for(iz=0; iz<fdm->nzpad; iz++) {
                atten = ro[iy][ix][iz];
                ua[iy][ix][iz] = ua[iy][ix][iz] * vp[iy][ix][iz]
                + 2*uo[iy][ix][iz]*atten
                - um[iy][ix][iz]*atten;
            }
	    }
	...
	if(dabc) {
	    ...
	    sponge3d_apply(ua,spo,fdm);
	}

    boundary_time += omp_get_wtime()-time;
    ...
    output_time += omp_get_wtime()-time;
    ...
    sf_warning("\n\n*************** TIME STATISTICS *************\n\n");
    sf_warning("total    : %3.5f seconds", final_time);
    sf_warning("stencil  : %3.5f seconds (%3.2f\\%)",stencil_time,stencil_time/final_time*100.0);
    sf_warning("injection: %3.5f seconds (%3.2f\\%)",injection_time,injection_time/final_time*100.0);
    sf_warning("step     : %3.5f seconds (%3.2f\\%)",step_time,step_time/final_time*100.0);
    sf_warning("boundary : %3.5f seconds (%3.2f\\%)",boundary_time,boundary_time/final_time*100.0);
    sf_warning("output   : %3.5f seconds (%3.2f\\%)",output_time,output_time/final_time*100.0);
